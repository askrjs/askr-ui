...
describe('SidebarLayout - Behavior', () => {
  ...
  it('should render the sidebar-layout data-slot on the root', () => {
    ...
    expect(container.querySelector('[data-slot="sidebar-layout"]')).not.toBeNull();
  });

  it('should render an aside element with the sidebar data-slot', () => {
    container = mount(<SidebarLayout sidebar={<span>Nav</span>}>Content</SidebarLayout>);
  ...
  it('should render sidebar content inside the aside slot', () => {
    container = mount(
      <SidebarLayout sidebar={<span id="nav-content">Nav</span>}>Content</SidebarLayout>,
    );
    const aside = container.querySelector('aside[data-slot="sidebar"]');
  ...
  it('should emit data-sidebar-position attribute', () => {
    container = mount(<SidebarLayout sidebarPosition="end" sidebar="Nav">Content</SidebarLayout>);
    const root = container.querySelector('[data-slot="sidebar-layout"]');
    expect(root?.getAttribute(SIDEBAR_LAYOUT_A11Y_CONTRACT.DATA_ATTRIBUTES.sidebarPosition)).toBe(
      'end',
    );
  });

  it('should emit data-sidebar-width attribute', () => {
    container = mount(<SidebarLayout sidebarWidth="20rem" sidebar="Nav">Content</SidebarLayout>);
    const root = container.querySelector('[data-slot="sidebar-layout"]');
    expect(root?.getAttribute(SIDEBAR_LAYOUT_A11Y_CONTRACT.DATA_ATTRIBUTES.sidebarWidth)).toBe(
      '20rem',
    );
  });
  ...
  it('should emit data-collapse-below attribute', () => {
    ...
    const root = container.querySelector('[data-slot="sidebar-layout"]');
    expect(root?.getAttribute(SIDEBAR_LAYOUT_A11Y_CONTRACT.DATA_ATTRIBUTES.collapseBelow)).toBe(
      'md',
    );
  });
});
